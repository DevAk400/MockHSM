...
module HSM
  class M2
    ...
    #   Input format      1 Numeric, "0" for bin, "1" for hex
    #   Output format     1 Numeric, similar to input format but with "2" for plaintext in addition
    ...
    #   Message length    4 Hexadecimal
    #   Encrypted message n Bytes / Hexadecimal depending on output format flag
    ...
    def call(client)
      ...
      inputFormat = client.read_nonblock(1) # binary or hex
      reject("03") unless ["0", "1"].include?(inputFormat)

      outputFormat = client.read_nonblock(1) #binary, hex, text
      reject("04") unless ["0", "1", "2"].include?(outputFormat)
      ...
      encryptedKey = client.read_nonblock(80).unHex
      begin
        decryptedKey = hsmdecrypt(encryptedKey, keyBlockHeader[0..11])  # only need 12 bytes for gcm encryption
      rescue
        reject("10")
      end
      bitSize = decryptedKey[0..1].unpack1("n")
      secretKey = decryptedKey[2..(1 + bitSize / 8)].b
      ...
      cipher = OpenSSL::Cipher.new(algorithm).decrypt  # Create cipher in encryption mode
      cipher.key = secretKey
      cipher.iv = iv if needIV
      cipher.padding = 0
      decrypted = cipher.update(message).b
      decrypted = decrypted + cipher.final.b
      decrypted = decrypted.b
      # hex if output mode requires
      decrypted = decrypted.toHex if outputFormat == "1"
      outLen = [decrypted.size()].pack("n").toHex

      payload = "M300#{hexIV.upcase}#{outLen}#{decrypted}"
      client.write(wrap(payload))
      client.close
    rescue ArgumentError => e
      payload = "M3#{e.message}"
      client.write(wrap(payload))
